...
st.set_page_config(
    page_title="個人情報とプライバシー",
    page_icon="🔒",
    layout="wide",
    initial_sidebar_state="collapsed"
)

st.title("個人情報とプライバシー（pp.215-216）")
...
if st.button("この写真にひそむリスクを確認する"):
    ...
    # 詳細解説を文章で表示
    st.warning("🏢 **背景の建物から撮影場所が特定される危険性（高リスク）**")
    st.write("有名な建物や特徴的な建造物が写っていると、撮影場所が簡単に特定されてしまいます。あなたがよく行く場所や住んでいる地域がバレる可能性があります。")
    
    st.warning("👔 **制服から学校が特定される危険性（高リスク）**")
    st.write("学校の制服は、その学校独特のデザインや色を持っています。制服が写ることで、どこの学校に通っているかが特定され、さらに個人情報が推測される可能性があります。")
    
    st.error("📍 **位置情報（ジオタグ）による場所の特定（最高リスク）**")
    st.write("スマートフォンで撮影した写真には、GPS情報が自動的に記録されることがあります。この位置情報（ジオタグ）により、撮影した正確な場所、時には自宅までもが特定される危険性があります。")
    
    st.warning("✌️ **ピースサインから指紋が読み取られる可能性（低リスク）**")
    st.write("高解像度の写真では、ピースサインをしている指先から指紋の模様が読み取られる可能性が指摘されています。技術的には可能ですが、現実的なリスクは比較的低いとされています。")
...
with tab1:
    ...
    if st.button("回答する", key="sns_answer"):
        if choice1:
            if "①" in choice1:
                st.success("✅ 正解です！")
            else:
...
with tab2:
    ...
    if st.button("回答する", key="flea_answer"):
        if choice2:
            if "①" in choice2:
                st.success("✅ 正解です！")
            else:
...
with tab3:
    ...
    if st.button("回答する", key="email_answer"):
        if choice3:
            if "②" in choice3:
                st.success("✅ 正解です！")
            else:
